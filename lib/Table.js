...
const { DescribeTableCommand, CreateTableCommand, UpdateTableCommand, DeleteTableCommand, DescribeTimeToLiveCommand } = require('@aws-sdk/client-dynamodb');
...
Table.prototype.deleteIndex = function deleteIndex(indexName) {
  ...
  var command = new UpdateTableCommand(params);
...
Table.prototype.describe = function (next) {
  ...
  var ddb = this.base.ddb();
  const command = new DescribeTableCommand(describeTableReq);
  ddb.send(command).then(function (data) {
    deferred.resolve(data);
  }).catch(function (err) {
    debug('error describing table', err);
...
Table.prototype.create = function (next) {
  ...
  const command = new CreateTableCommand(createTableReq);
  ddb.send(command).then(function (data) {
...
Table.prototype.delete = function (next) {
  ...
  const command = new DeleteTableCommand(deleteTableReq);
  ddb.send(command).then(function (data) {
