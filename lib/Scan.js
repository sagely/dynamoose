...
Scan.prototype.exec = function (next) {
  ...
  function toModel (item) {
    ...
    function scanByRawFilter() {
      ...
      var command = new libDynamodb.ScanCommand(scanReq);

      dbClient.send(command).then(function(data) {
        if (!data) {
  ...
  function scanSegment (segment) {
    ...
    function scanOne() {
      ...
      var ddb = Model.$__.base.ddb();
      const command = new ScanCommand(scanOneReq);
