...
Model.get = function(NewModel, key, options, next) {
  ...
  function get () {
    ...
    var ddb = newModel$.base.ddb();
    const command = new GetItemCommand(getReq);
...
Model.update = function(NewModel, key, update, options, next) {
  ...
  function updateItem () {
    ...
    var ddb = newModel$.base.ddb();
    const command = new UpdateItemCommand(updateReq);
    ddb.send(command).then(function (data) {
...
Model.prototype.delete = function(options, next) {
  ...
  function deleteItem() {
    ...
    var ddb = model$.base.ddb();
    const command = new DeleteItemCommand(getDelete);
    ddb.send(command).then(function (data) {
...
Model.batchGet = function(NewModel, keys, options, next) {
  ...
  function batchGet () {
    ...
    var ddb = newModel$.base.ddb();
    const command = new BatchGetItemCommand(batchReq);
    ddb.send(command).then(function (data) {
...
function batchWriteItems (NewModel, batchRequests) {
  ...
  var batchList = batchRequests.map(function (batchReq) {
    ...
    var ddb = newModel$.base.ddb();
    const command = new BatchWriteItemCommand(batchReq);
    ddb.send(command).then(function (data) {
      deferredBatch.resolve(data);
    }).catch(function (err) {
      debug('Error returned by batchWriteItems', err);
      deferredBatch.reject(err);
    });
